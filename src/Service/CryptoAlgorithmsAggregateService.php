...
namespace BlackCat\Database\Packages\CryptoAlgorithms\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto;
use BlackCat\Database\Packages\CryptoAlgorithms\Mapper\CryptoAlgorithmDtoMapper;
 ...
 */
final class CryptoAlgorithmsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CryptoAlgorithmRepository $cryptoAlgorithmRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'crypto_algorithms' => $this->cryptoAlgorithmRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\CryptoAlgorithms\ModuleException("Record {$idStr} not found for locking.");
            }
