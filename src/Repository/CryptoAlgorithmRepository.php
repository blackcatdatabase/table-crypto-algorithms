...
namespace BlackCat\Database\Packages\CryptoAlgorithms\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\CryptoAlgorithms\Definitions;
use BlackCat\Database\Packages\CryptoAlgorithms\Criteria;
use BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto as Dto;
use BlackCat\Database\Packages\CryptoAlgorithms\Mapper\CryptoAlgorithmDtoMapper as RowMapper;
...
final class CryptoAlgorithmRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\CryptoAlgorithms\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'nist_level', 'status', 'params' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'class', 'name', 'variant' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\CryptoAlgorithms\Definitions::table());
            return $bulk->upsertMany($rows, [ 'class', 'name', 'variant' ], [ 'nist_level', 'status', 'params' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null */
    public function getByClassAndNameAndVariantNorm(string $class, string $name, string $variantNorm, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null {
        $row = $this->getByUnique([ 'class' => $class, 'name' => $name, 'variant_norm' => $variantNorm ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CryptoAlgorithms\Mapper\CryptoAlgorithmDtoMapper::fromRow($row);
    }
    public function existsByClassAndNameAndVariantNorm(string $class, string $name, string $variantNorm): bool {
        $where = 't.' . Ident::q($this->db, 'class') . ' = :uniq_class' . ' AND ' . 't.' . Ident::q($this->db, 'name') . ' = :uniq_name' . ' AND ' . 't.' . Ident::q($this->db, 'variant_norm') . ' = :uniq_variant_norm';
        return $this->exists($where, [ 'uniq_class' => $class, 'uniq_name' => $name, 'uniq_variant_norm' => $variantNorm ]);
    }
    /** @return int|string|null */
    public function getIdByClassAndNameAndVariantNorm(string $class, string $name, string $variantNorm) {
        $row = $this->getByClassAndNameAndVariantNorm($class, $name, $variantNorm);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null */
    public function getByClassAndNameAndVariant(string $class, string $name, string $variant, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null {
        $row = $this->getByUnique([ 'class' => $class, 'name' => $name, 'variant' => $variant ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CryptoAlgorithms\Mapper\CryptoAlgorithmDtoMapper::fromRow($row);
    }
    public function existsByClassAndNameAndVariant(string $class, string $name, string $variant): bool {
        $where = 't.' . Ident::q($this->db, 'class') . ' = :uniq_class' . ' AND ' . 't.' . Ident::q($this->db, 'name') . ' = :uniq_name' . ' AND ' . 't.' . Ident::q($this->db, 'variant') . ' = :uniq_variant';
        return $this->exists($where, [ 'uniq_class' => $class, 'uniq_name' => $name, 'uniq_variant' => $variant ]);
    }
    /** @return int|string|null */
    public function getIdByClassAndNameAndVariant(string $class, string $name, string $variant) {
        $row = $this->getByClassAndNameAndVariant($class, $name, $variant);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null {
