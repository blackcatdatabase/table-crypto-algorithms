...
use BlackCat\Database\Packages\CryptoAlgorithms\Repository\CryptoAlgorithmRepositoryInterface;
...
class CryptoAlgorithmRepository implements CryptoAlgorithmRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'class', 'name', 'variant' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\CryptoAlgorithms\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'nist_level', 'status', 'params' ]);
          return count($rows);
        }
    ...
    public function getByClassAndNameAndVariantNorm(string $class, string $name, string $variantNorm, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null {
        return $this->getByUnique([ 'class' => $class, 'name' => $name, 'variant_norm' => $variantNorm ], $asDto);
    }
    ...
    public function getIdByClassAndNameAndVariantNorm(string $class, string $name, string $variantNorm) {
        $row = $this->getByClassAndNameAndVariantNorm($class, $name, $variantNorm, false);
    ...
    public function getByClassAndNameAndVariant(string $class, string $name, string $variant, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoAlgorithms\Dto\CryptoAlgorithmDto|null {
        return $this->getByUnique([ 'class' => $class, 'name' => $name, 'variant' => $variant ], $asDto);
    }
    ...
    public function getIdByClassAndNameAndVariant(string $class, string $name, string $variant) {
        $row = $this->getByClassAndNameAndVariant($class, $name, $variant, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
