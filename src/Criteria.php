...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'class', 'name', 'variant', 'variant_norm', 'nist_level', 'status', 'params', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'class', 'name', 'variant', 'variant_norm', 'status' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'class', 'name', 'variant', 'variant_norm', 'nist_level', 'status', 'created_at' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\CryptoAlgorithms\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\CryptoAlgorithms\Definitions::softDeleteColumn();
